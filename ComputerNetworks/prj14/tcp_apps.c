...
void *tcp_server(void *arg)
{
	...
	if(file_mode)
	{
		...
		int rlen;
		char tmp_buf[1000];
		long long unsigned int recv_len = 0;
...
void *tcp_client(void *arg)
{
	...
	if(file_mode)
	{
		...
		while(1)
		{
			...
			memset(tmp_buf, 0, SEND_PAC_LEN);
			// data_len = min(1000, (file_len - sent_len));
			
			data_len = 0;
			while((feof(fp) == 0) && (data_len < SEND_PAC_LEN))
			{
	...
	// prj14 added
	FILE *record_file;
	record_file = fopen("cwnd_record.txt", "w");

	for(int index1 = 0; index1 < record_index; index1++)
	{
