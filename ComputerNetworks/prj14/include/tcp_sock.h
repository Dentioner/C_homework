...
#define TCP_MAX_BACKLOG 128
	...
	struct synch_wait *wait_ofo; // prj13 added
	...
	u32 cwnd_state;
	u32 dupacks;
	pthread_mutex_t cwnd_lock;

};

typedef struct send_buf_element // prj13 added
{
	int len;
	char *packet;
	u32 seq_end;
...
typedef struct rcv_ofo_element // prj13 added
{
	int pl_len;
	char *payload;
	u32 seq;
...
#define CWND_RECORD_NUM 10000

#define MSS (1514 - ETHER_HDR_SIZE - IP_BASE_HDR_SIZE - TCP_BASE_HDR_SIZE)
#define INIT_SSTHRESH 100

// prj14 added
u32 cwnd_record[CWND_RECORD_NUM]; // 记录cwnd数据的数组
int record_index;
...
int tcp_update_send_buf(struct tcp_sock *tsk, struct tcp_cb *cb); // prj13 added
void write_ofo_buffer(struct tcp_sock *tsk, struct tcp_cb *cb); // prj13 added
int tcp_update_cwnd(struct tcp_sock *tsk, struct tcp_cb *cb); // prj14 added
void resend_send_buf(struct tcp_sock *tsk); // prj14 added
#endif
