...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
	...
	tcp_add_send_buf(tsk, packet, len); // 在这个函数调用add send buf是在发送未确认的数据包
	if(tsk->retrans_timer.enable == 0) // prj13 added
	{
	...
	{
		printf("Error. timer has not been set up yet.\n");
	}
	*/

	// printf("debug: in tcp send packet.\n");

	// printf("debug: before ip_send_packet in tcp_send_packet.\n");
	ip_send_packet(packet, len);
	// printf("debug: after  ip_send_packet in tcp_send_packet.\n");
// debug
	int elem_counter_debug = 0;
	snd_buf_elem *snd_p, *snd_q;
	list_for_each_entry_safe(snd_p, snd_q, &(tsk->send_buf), list)
	{
		elem_counter_debug++;
	}

	// printf("debug: seq:%u, ack:%u, len:%u, sndbuf:%d\n", seq, ack, tcp_data_len, elem_counter_debug);
}
...
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags)
{
	...
	if (flags & (TCP_SYN|TCP_FIN))// prj13 added
	{
		tsk->snd_nxt += 1;
		tcp_add_send_buf(tsk, packet, pkt_size); // 在这个函数调用add send buf是在发送未确认的SYN包或FIN包
		if(tsk->retrans_timer.enable == 0) 
		{
			// printf("debug: before set retrans timer.\n");
			// sleep(2); // debug	
			tcp_set_retrans_timer(tsk);
			// printf("debug: after set retrans timer.\n");
		}
	}

	

	// printf("debug: in tcp send control packet.\n");
	// printf("debug: tsk snd nxt=%d\n", tsk->snd_nxt);

	

	// printf("debug: before ip send packet.\n");
