...
{
	int len = ntohs(ip->tot_len) - IP_HDR_SIZE(ip) - TCP_HDR_SIZE(tcp); // tcp正文长度（字节流的字节个数）
...
void handle_tcp_packet(char *packet, struct iphdr *ip, struct tcphdr *tcp)
{
	...
	// debug
	/*
	static int init_seq = 0;
	static int init_ack = 0;
	if(init_seq == 0)
		init_seq = cb.seq;
	if(init_ack == 0)
		init_ack = cb.ack;
	printf("debug: relative:seq=%u, ack=%u, abs:seq=%u, len=%u\n", (cb.seq - init_seq), (cb.ack - init_ack), cb.seq, cb.pl_len);
	*/
