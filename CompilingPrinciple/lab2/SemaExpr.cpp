  ...
  if(elementWiseON) // check #pragma elementWise is written
  {
    ...
    // check LHS & RHS both are ConstantArrayType
    {
      ...
      if((LHS_elemType == RHS_elemType) && (LHS_elemType->isIntType_lab2()))
      {
        ...
        if(LHS_elem_num == RHS_elem_num)
        {
          // check if the LHS & RHS is RValue or not
          if(!LHS_expr->isRValue())
          {
            Qualifiers t;
            ImplicitCastExpr *LHS_rvalue = ImplicitCastExpr::Create(Context, Context.getUnqualifiedArrayType(LHS_expr->getType().getUnqualifiedType(), t), CK_LValueToRValue, const_cast<Expr *>(LHS_expr), 0, VK_RValue);
            LHS = LHS_rvalue;
          }
          ...
          {
            Qualifiers t;
            ImplicitCastExpr *RHS_rvalue = ImplicitCastExpr::Create(Context, Context.getUnqualifiedArrayType(RHS_expr->getType().getUnqualifiedType(), t), CK_LValueToRValue, const_cast<Expr *>(RHS_expr), 0, VK_RValue);
            RHS = RHS_rvalue;
          }
