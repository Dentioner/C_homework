...
// following are generating IR code for elementWise
  if(E->getType()->getTypeClass() == Type::ConstantArray){
    if(BinaryOperator::classof(E)){
      ...
      //分配for循环的各个块
      llvm::BasicBlock *cond_block = createBasicBlock("elementWise_for_cond_test");
      llvm::BasicBlock *compound_block = createBasicBlock("elementWise_compound_stmts");
      llvm::BasicBlock *loopva_update_block = createBasicBlock("elementWise_loopva_update");
      llvm::BasicBlock *exit_block = createBasicBlock("elementWise_exit");

      //设置cond_test块，对于三种情况，都是相同的，因为有相同的循环索引变量
      EmitBlock(cond_block);
      llvm::LoadInst *idx_i = Builder.CreateLoad((llvm::Value*)Alloc, "");
      idx_i->setAlignment(4);
      llvm::Value *idx = Builder.CreateIntCast(idx_i, IntPtrTy, false, "idxprom");
      //idx->setAlignment(4);
      //llvm::Value *idxPromoted = Builder.CreateIntCast(idx, IntPtrTy, false, "idxprom");
      //读取数组的大小并比较
      const ConstantArrayType *const_array_type = dyn_cast<ConstantArrayType>(E->getType().getUnqualifiedType());
      //llvm::Value *array_bound = dyn_cast<llvm::Value *>(const_array_type->getSize());
      llvm::Value *array_bound = llvm::ConstantInt::get(IntPtrTy,const_array_type->getSize());
      llvm::Value *less_comp = Builder.CreateICmpSLT(idx, array_bound);
      Builder.CreateCondBr(less_comp, compound_block, exit_block);

      EmitBlock(compound_block);
      //设置compound块，这个块包含了所有的运算
      ...
      if(BinaryOperator::classof(rhs)){
        ...
        Expr *lhs1 = bo1->getLHS(), *rhs1 = bo1->getRHS();
        ...
        //获得A，B，C 的指针
        llvm::Value *arrayPtrC = MakeAddrLValue(baseC, T, Alignment).getAddress();
        llvm::Value *arrayPtrA = MakeAddrLValue(baseA, T, Alignment).getAddress();
        llvm::Value *arrayPtrB = MakeAddrLValue(baseB, T, Alignment).getAddress();
        ...
        if(bo1->getOpcode() == BO_Add)
          result = Builder.CreateAdd((llvm::Value*)valueA, (llvm::Value*)valueB, "add");
        else if(bo1->getOpcode() == BO_Mul)
          result = Builder.CreateMul((llvm::Value*)valueA, (llvm::Value*)valueB, "mul");
      ...
      //说明是赋值运算C=A
        ...
        const DeclRefExpr *declRef1 = dyn_cast<DeclRefExpr>(rhs);
        const ValueDecl *decl1 = declRef1 -> getDecl();
        baseA = LocalDeclMap.lookup((Decl*)decl1);

        const ValueDecl *VD = declRef1->getDecl();
        CharUnits Alignment = getContext().getDeclAlign(VD);

        QualType T = declRef1->getType();
        llvm::Value *arrayPtrC = MakeAddrLValue(baseC, T, Alignment).getAddress();
        llvm::Value *arrayPtrA = MakeAddrLValue(baseA, T, Alignment).getAddress();
      ...
      Builder.CreateBr(loopva_update_block);

      // loop ref variable update, using inc
      EmitBlock(loopva_update_block);
      llvm::LoadInst *inc_load = Builder.CreateLoad((llvm::Value*)Alloc, "");
      inc_load->setAlignment(4);
      llvm::Value *const_one = llvm::ConstantInt::get(Int32Ty, 1);
      llvm::Value *inc_result = Builder.CreateAdd((llvm::Value*)inc_load, const_one, "");
      llvm::StoreInst *inc_store = Builder.CreateStore(inc_result, (llvm::Value*)Alloc, false);
      inc_store->setAlignment(4);
      Builder.CreateBr(cond_block);

      //exit block
      EmitBlock(exit_block);
