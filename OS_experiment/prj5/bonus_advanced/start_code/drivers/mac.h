...
#define PNUM (256)   // 64 packages
#define HALF_PNUM (128)
...
desc_t tx_desc_list[1];
desc_t rx_desc_list[PNUM];

//char send_package[PNUM][PSIZE];
//char recv_package[PNUM][PSIZE];

uint32_t send_package[1][PSIZE];
uint32_t recv_package[PNUM][PSIZE];

uint32_t task2_print_buffer[PNUM][PSIZE];
uint32_t task2_rdes0[PNUM];

int task2_print_buffer_index;
