...
/*******************************debug*****************************************/
    ...
    desc_seg = (desc_seg + 1)%2;

    recv_num_now += HALF_PNUM;
    ...
    if(desc_seg == 1)
    {
        for (index1 = 0; index1 < HALF_PNUM; index1++)
        {

            rx_desc_list[index1].tdes0 = 0x80000000;
    ...
    else
    {
        for (index1 = 0; index1 < HALF_PNUM; index1++)
        {
            ...
            rx_desc_list[index1 + HALF_PNUM].tdes0 = 0x80000000;
            
        }

    }

    reg_write_32(DMA_BASE_ADDR + DmaRPD, HALF_PNUM);
...
/*******************************debug*****************************************/    
    ...
    sys_move_cursor(1, 9);
    printf("recv valid %d packages!:", PNUM);
...
void do_print_buffer()
{
    ...
    for(index1 = 0; index1 < PNUM; index1++)
    {
        ...
        printk("%d recv buffer, r_desc = 0x%x:\n", index1, rx_desc_list[index1].tdes0);

        vt100_move_cursor(1, 4);
        for(index2 = 0; index2 < PSIZE; index2++)
        {
            tmp_data = (uint32_t *)(daddr + index1*0x400);
            if (tmp_data[index2])
            {    
                //printk("%x ", task2_print_buffer[index1][index2]);
                //printk("%x ", recv_package[index1][index2]);
                printk("%x ", tmp_data[index2]);
                index3++;
