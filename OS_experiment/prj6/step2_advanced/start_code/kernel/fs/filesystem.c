...
dentry_t tmp_dentry_arr[DIRECT_DENTRY_ARR_LENGTH];
...
void load_dentry_arr(inode_t * inode)
{
    int index1;
    dentry_t * tmp_pointer = tmp_dentry_arr;
    for (index1 = 0; index1 < DIRECT_LINK_NUM; index1++)
    {
        sdread(read_block_buffer, (START_ADDRESS_SD + (inode->direct_blocks[index1])*BLOCK_SIZE), BLOCK_SIZE);
        os_memcpy((void *)tmp_pointer, (void *)read_block_buffer, (DENTRY_NUM_PER_BLOCK*sizeof(dentry_t)));
        tmp_pointer += DENTRY_NUM_PER_BLOCK;
    }

}

void write_dentry_arr(inode_t *inode)
{
    int index1;
    dentry_t * tmp_pointer = tmp_dentry_arr;
    for (index1 = 0; index1 < DIRECT_LINK_NUM; index1++)
    {
        if (inode->direct_blocks[index1] != 0)
        {
            os_memcpy((void *)read_block_buffer, (void *)tmp_pointer, (DENTRY_NUM_PER_BLOCK*sizeof(dentry_t)));
            sdwrite(read_block_buffer, (START_ADDRESS_SD + (BLOCK_SIZE * inode->direct_blocks[index1])), BLOCK_SIZE);
            tmp_pointer += DENTRY_NUM_PER_BLOCK;
        }
    }
}
...
/********************************************************************************/   
        ...
        for(index2 = 0; index2 < DIRECT_DENTRY_ARR_LENGTH; index2 ++)
        {
        ...
        os_memcpy((void *)read_block_buffer, (void *)tmp_dentry_arr, BLOCK_SIZE);
...
void do_mkdir(uint32_t arg_filename)
{
    ...
    load_dentry_arr(tmp_parent_inode_p);
    ...
    if(index2 == DIRECT_DENTRY_ARR_LENGTH)
    {
        printk("ERROR.....\n");
        while(1);
    }
    ...
    write_dentry_arr(tmp_parent_inode_p);
...
void do_rmdir(uint32_t arg_filename)
{
    ...
    for (index4 = 0; index4 < DIRECT_LINK_NUM; index4++)
        delete_blk_num[index4] = tmp_inode1_p->direct_blocks[index4]; // 先记住，要不然等下就没了
    ...
    int index5;
    dentry_t * tmp_pointer5 = tmp_dentry_arr;
    for (index5 = 0; index5 < DIRECT_LINK_NUM; index5++)
    {
        if(delete_blk_num[index5] != 0)
        {
            os_memcpy((void *)read_block_buffer, (void *)tmp_pointer5, (DENTRY_NUM_PER_BLOCK*sizeof(dentry_t)));
            sdwrite(read_block_buffer, (START_ADDRESS_SD + (BLOCK_SIZE * delete_blk_num[index5])), BLOCK_SIZE);
            tmp_pointer5 += DENTRY_NUM_PER_BLOCK;
        }
    }
...
void do_cd(uint32_t arg_filename)
{
    ...
    for(index1 = 0; index1 < tmp_path_depth; index1++)
    {
        ...
        { // 注意，由于上级相对寻址的pattern含有本级相对寻址，因此先检查上级比较合适
            ...
            load_dentry_arr(tmp_inode2_p); // 将上层目录加载出来
            // 更新绝对路径
            tmp_abs_path_depth--;
            for(index2 = 0; index2 < FILENAME_LENGTH; index2++)
            {
        ...
        else if ((!my_strncmp((char *)&(tmp_path[index1]), "~", 1)) && (index1 == 0))// 绝对寻址
        {
            ...
            load_dentry_arr(tmp_inode2_p); // 加载根目录
            // 更新绝对路径
        ...
        else // 绝对寻址
        {
            ...
            load_dentry_arr(tmp_inode2_p); // 将下一层目录加载出来
...
{   
    ...
    load_dentry_arr(tmp_inode_p);

    for(index1 = 0, index3 = 0; index1 < DIRECT_DENTRY_ARR_LENGTH; index1++)
    {
...
void do_touch(uint32_t arg_filename)
{
    ...
    load_dentry_arr(tmp_parent_inode_p);
    // check if the new filename exists
    for (index1 = 0; index1 < DIRECT_DENTRY_ARR_LENGTH; index1 ++)
    {
